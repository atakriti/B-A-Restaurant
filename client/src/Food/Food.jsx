...
function Food() {
  let { products,users,signinValue,isSignedIn,fetchUsers,setUsers } = useContext(context)
  let { type } = useParams()
  ...
  console.log("🚀 ~ file: Food.jsx:14 ~ Food ~ foundUserState", foundUserState)
  let [foundMealState,setFoundMealState] = useState()
  console.log("🚀 ~ file: Food.jsx:15 ~ Food ~ foundMealState", foundMealState)
  // ============================== Plus =========================
  let handlePlus = () => {
    setFoundMealState({
      ...foundMealState,
  ...
  let handleAddToCart =async () => {
    ...
    if (foundUserState.cart.some((item) => item?._id === foundMealState?._id)) {
      ...
      setFoundUserState({ ...foundUserState, cart: sameFood,cartArchive:sameFood });
      await axios.put(`http://localhost:4000/updateUser/${foundUserState?._id}`,{ ...foundUserState, cart: sameFood,cartArchive:sameFood });
  ...
  return (
    <div className='food_'>
      ...
      <div className="food_container">
        {findType.map((item,i) => (
          <div onMouseEnter={()=>setFoundMealState(item)} className={"food_box"}>
            {/* ================== Front =============== */}
            <div className={"front_"}>
            <a  className='foodImg'><img src={`http://localhost:4000${item.image}`} alt="" /></a>
            ...
            <div  className="back_">
              <div className="back_container">
                ...
                <div className="inner_back_container">
                  <a><img src={cart_logo} alt="" /></a>
                  <h2>Ingredients</h2>
              <ul>{item.ing.split(",").map(item => <li>{item}</li>)}</ul>
